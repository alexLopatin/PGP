...
int main(int argc, char* argv[])
{
	...
	auto errors = new double[threadCount];
	...
	do
	{
		...
		#pragma omp parallel
		{
			...
			auto linearId = threadId;

			auto i = linearId % BlockSize[0];
			auto j = (linearId / BlockSize[0]) % BlockSize[1];
			auto k = (linearId / BlockSize[0]) / BlockSize[1];

			while (linearId < BlockSize[0] * BlockSize[1] * BlockSize[2])
			{
				next[GetIndex(i, j, k)] =
					...
					/ (2 * (1 / h2x + 1 / h2y + 1 / h2z));
				errors[threadId] = Max(errors[threadId], Abs(next[GetIndex(i, j, k)] - data[GetIndex(i, j, k)]));

				linearId += threadCount;

				i = linearId % BlockSize[0];
				j = (linearId / BlockSize[0]) % BlockSize[1];
				k = (linearId / BlockSize[0]) / BlockSize[1];
			}
			//Перерасчет
			/*for (int i = 0; i < BlockSize[0]; i++)
				for (int j = 0; j < BlockSize[1]; j++)
